...
用于管理H3、Grok等视频模型的短剧镜头任务。可按模型切换专用提示词、上传成片，并从视频开头和结尾附近自动挑选清晰、非黑屏的首帧和尾帧。处理完成后自动删除上传的原视频。
...
- EP02《明天的雨》生产包：9个10秒镜头、90秒完整状态链、H3/Grok逐镜策略和6组新增固定资产
- 模型分轨存储：任务状态、成片、首帧、尾帧互不覆盖
- 按秒拆分的专业提示词、参考素材、对白与连续性记录
- 统一视觉指纹：跨集锁定3D写实、年代色盘、PBR材质与光线逻辑，昼夜和天气可按剧情有依据地变化
- 场景三层状态：母版锁空间骨架，状态账本锁持续道具，真实尾帧锁镜头动作结果
- 跨集开拍门禁：可提前制作独立资产，但上一集验收前，下一集连续性提示词仅标记为预制作
...
首次启动会载入《重回1998》第一集10个任务和第二集9个任务；已有部署升级到V1.16.3时会自动补入第二集，保留原有制作状态与动态首尾帧。
...
- [多模型生产与首尾帧通用规范](docs/05_多模型生产与首尾帧通用规范.md)
- [H3与Grok模型执行规范](docs/11_H3与Grok模型执行规范.md)
...
- [第二集镜头连续性与生产交接](docs/12_EP02镜头连续性与生产交接.md)
...
- `tmp/uploads/`：上传处理时的临时视频，成功或失败后都会自动删除
